...
import { listCustomers, getCustomerLedger, recordCustomerPayment } from "../api/customers";
import type { CustomerDto, CustomerLedgerDto } from "../api/customers";
...
export default function CustomerDetailPage() {
  ...
  async function loadData() {
    ...
    setError(null);
    try {
      const [customers, ledgerData] = await Promise.all([listCustomers(), getCustomerLedger(id)]);
  ...
  if (error || !customer || !ledger) {
    return (
      <EmptyState
        title="Something went wrong"
        description={error ?? "Customer not found."}
        action={
          <Link to="/customer-udhaar">
            <Button variant="secondary" size="sm">Back to Customer Udhaar</Button>
          </Link>
        }
      />
    );
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <Link to="/customer-udhaar" className="flex items-center gap-1 text-sm text-ink-500 hover:text-ink-700">
        <ArrowLeft className="h-4 w-4" /> Back to Customer Udhaar
      </Link>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card>
          <CardBody>
            ...
            <p className="mt-1 text-xl font-semibold text-ink-900">
              {formatCurrency(ledger.balance.total_sales, currencyCode)}
            </p>
          </CardBody>
        </Card>
        <Card>
        ...
        <Card>
          <CardBody>
            <p className="text-sm text-ink-500">Outstanding Balance</p>
        ...
        <CardBody>
          {ledger.entries.length === 0 ? (
            <EmptyState
              ...
              description="Sales and payments for this customer will appear here."
            />
          ) : (
      ...
      <RecordPaymentModal
        ...
        onSubmit={async (payload) => {
          await recordCustomerPayment(customer.id, payload);
